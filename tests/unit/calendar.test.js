...
describe('calendar module', () => {
  ...
  it('builds schedules with conditional medication and warning events', () => {
    ...
    const plenvuSchedule = buildSchedule(examDate, 'en', false, 'plenvu', false, false);
    ...
    const fullSchedule = buildSchedule(examDate, 'en', true, 'citrafleet', true, true);
    ...
    for (let i = 1; i < fullSchedule.length; i += 1) {
      expect(fullSchedule[i - 1].dateTime.getTime()).toBeLessThanOrEqual(
        fullSchedule[i].dateTime.getTime()
      );
    }
  });
  ...
  it('renders hero summary with iron, anticoag and dulcolax rows', () => {
    const elements = createHeroElements();
    const examDate = new Date(2026, 1, 22, 8, 30);
    const schedule = buildSchedule(examDate, 'en', true, 'citrafleet', true, true);
  ...
  it('renders timeline cards and empty state message', () => {
    ...
    const schedule = buildSchedule(new Date(2026, 1, 22, 8, 30), 'en', false, 'moviprep', false, false);
  ...
  it('renders calendar notes with personalized medication label', () => {
    ...
    renderCalendarInfo(
      { calendarInfo },
      buildSchedule(new Date(2026, 1, 22, 8, 30), 'en', true, 'plenvu', false, false),
      'en',
      {
        medication: 'plenvu',
        isConstipated: true,
  ...
  it('downloads ICS file with timed reminders and exam location', async () => {
