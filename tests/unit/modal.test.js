...
describe('modal module', () => {
  ...
  const setupDom = () => {
    document.body.innerHTML = `
      <div id="videoModal" aria-hidden="true">
        <button id="closeBtn" data-modal-close>Close</button>
        <div id="modalBody"></div>
      </div>
      <div id="videoGrid">
        <button id="videoBtn" data-video-id="video-1">Open</button>
      </div>
    `;
  ...
  it('opens video placeholder content only for valid video buttons', () => {
    ...
    elements.videoGrid.innerHTML = '<button id="unknown" data-video-id="missing">Unknown</button>';
  ...
  it('covers non-closing clicks, non-escape keys and missing video grid fallback', () => {
    ...
    elements.videoGrid.innerHTML = '<button id="missingVideo" data-video-id="does-not-exist">Missing</button>';
